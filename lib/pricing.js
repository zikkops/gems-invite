...
// A pledge activation holds two mutually exclusive spots, so it owns two
// selection keys — `<id>_p` for the presenting spot, `<id>_g` for a supporting
// one — neither of which is the activation id itself.
export const pledgeKeys = (a) => ({ p: `${a.id}_p`, g: `${a.id}_g` });
...
export function normalizeSelection(raw) {
  ...
  // The presenting spot wins if a tampered payload somehow claims both.
  for (const a of activations) {
    if (a.kind !== 'pledge') continue;
    const { p, g } = pledgeKeys(a);
    if (Number(raw[p]) > 0) out[p] = 1;
    else if (Number(raw[g]) > 0) out[g] = 1;
  }
...
export function lineItems(selection) {
  ...
  for (const a of activations) {
    if (a.kind === 'pledge') {
      const { p, g } = pledgeKeys(a);
      if (selection[p])
        lines.push({ name: `${a.name} — ${a.presentingLabel}`, qty: 1, total: a.amount });
      if (selection[g])
        lines.push({ name: `${a.name} — ${a.guardianLabel}`, qty: 1, total: a.guardianAmount });
      continue;
    }
